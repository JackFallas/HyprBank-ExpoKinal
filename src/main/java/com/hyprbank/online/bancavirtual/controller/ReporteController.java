...
import com.hyprbank.online.bancavirtual.service.ReporteServicio;
 ...
 * @RequestMapping("/api/reportes") define la ruta base para todos los endpoints de este controlador.
 */
@RestController
@RequestMapping("/api/reportes")
public class ReporteController { // Renombrado a ReporteController

    private final ReporteServicio reporteServicio;

    /*
     * Constructor para la inyeccion de dependencias.
     * Spring inyectara la instancia de ReporteServicio.
     */
    @Autowired
     ...
     */
    @GetMapping("/accesos-usuarios/pdf")
    public ResponseEntity<byte[]> generarReporteAccesosUsuariosPdf() {
        try {
            // Llama al servicio para generar el reporte como un arreglo de bytes.
            byte[] pdfBytes = reporteServicio.generarReporteAccesosUsuariosPdf();

            // Configura las cabeceras de la respuesta HTTP para la descarga del PDF.
            HttpHeaders headers = new HttpHeaders();
            headers.setContentType(MediaType.APPLICATION_PDF);
            headers.setContentDispositionFormData("filename", "reporte_accesos_usuarios.pdf");
